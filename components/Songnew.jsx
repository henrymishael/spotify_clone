...
function Songnew({order, track, setView}) {
  ...
  return (
    ...
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className='text-neutral-400 grid grid-cols-2 py-4 px-5 hover:bg-white hover:bg-opacity-10 rounded-lg cursor-pointer'>
      <div  className='flex items-center space-x-4'>
        ...
        <div>
            <p className='text-white sm:w-36 lg:w-64 truncate'>{track.name}</p>
            <p 
            onClick={()=>{
                setView=("artist")
            ...
            className='w-40 hover:underline'>{track.artists[0].name}</p>
        </div>
      </div>
      <div className='flex items-center justify-between ml-auto md:ml-0'>
        <p className='w-40 sm:hidden md:block '>{track.album.name}</p>
